%% Setup for parfor on HPC

% myCluster = parcluster('local');
% if strcmp(getenv('PBS_ENVIRONMENT'),'PBS_BATCH')
%     %  if running on cluster/grid, changing storage location for parallel pool.
%     myCluster.JobStorageLocation = getenv('TMPDIR');     % TMPDIR is created by grid software
%     randWaitTime = 1+10*rand();
%     pause(randWaitTime);
% end
% parpool(myCluster,16);
...
for n=1:3
    ...
    %filename = ['C:\Users\mab679\Dropbox\Newcastle\Diffusion model\Data\GS054-',num2str(files(n)),'_p1_C60_1_depth_decimal place.TXT'];
    ...
    if n==1||n==2
        ...
        elseif m==3|| m==4
            opts.StartPoint = [0.05 1 0.12 0 0 0.5 360];
        end
    elseif n==3
        if m==1 || m==2
    ...
    opts.Lower = [-Inf -Inf 0.05 0 -Inf 0.2 -Inf];
    opts.Upper = [Inf Inf 0.7 10 Inf 0.8 Inf];
    opts.Weights = weight;
    ...
    if n==1
        a_vec.(T_n{n})=linspace(0,0.1,80);
        c_vec.(T_n{n})=linspace(0,0.12,40);
        b_vec.(T_n{n})=linspace(0.01,0.5,40);
        u0_vec.(T_n{n})=linspace(0.2,0.8,40);
    elseif n==2
        a_vec.(T_n{n})=linspace(0.1,0.25,80);
        c_vec.(T_n{n})=linspace(0,0.3,40);
        b_vec.(T_n{n})=linspace(0.01,0.15,40);
        u0_vec.(T_n{n})=linspace(0.6,0.8,40);
    elseif n==3
        a_vec.(T_n{n})=linspace(0.6,1,80);
        c_vec.(T_n{n})=linspace(0.75,1.2,40);
        b_vec.(T_n{n})=linspace(0.05,0.16,40);
        u0_vec.(T_n{n})=linspace(0.55,0.7,40);
            ...
            for n_b=1:N_b
                for n_u0=1:N_u0
                    for n_x0=1:N_x0
                        ...
                        y_est=Diffusion_numeric_1D_mid(xData-x0_vec_T(n_x0),D_gen,flip,delta_t,L_size,N_x,T_time)+c0;
    ...
    % Save data
    save('/home/mab679/output_F_HPC_norm_2.mat')
